 ...
 * @author Yang
 */
public class DataStreamWindowFunction extends FlinkApplication {

    /**
     * 从socket中获取数据
     */
    private static DataStream<String> data = sEnv.socketTextStream("127.0.0.1", 7777);
     ...
     */
    private static void processWindowFunction() {
        data.flatMap(new MyFlatMapFunction())
     ...
     * 加工，从而实现增量聚合。
     */
    private static void incrementalWindowAggregationWithReduceFunction() {
     ...
     */
    private static void incrementalWindowAggregationWithAggregateFunction() {
        data.flatMap(new MyFlatMapFunction())
                // 选取键，由于下方使用的是process function，这里用的是元组，所以必须通过KeySelector指定键
                .keyBy(t -> t.f0)
                // 设置时间窗口的大小为5秒
    ...
    public static void main(String[] args) throws Exception {
        ...
        sEnv.execute("window function demo");
    }
}
